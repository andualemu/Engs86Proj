...
class Biology9 extends Component {
  constructor(props) {
    super(props);
    this.state = {
      aRed: false,
  ...
  checkAnswer = (userAnswer, userAnswerLetter) => {
    ...
    // check the answer
    const questions = this.props.questions[8];
    ...
    if (userAnswer === this.props.questions[8].answer) {
      console.log('correct');
      recordScore();
    } else {
  ...
  explanDisplay = () => {
    ...
    return (
      <View>
        <Text> Answer: { questions[8].answer } </Text>
        <Text> { questions[8].explanation } </Text>
      </View>
    );
  }
    ...
    return (
      <View style={styles.container}>
        <CardFlip style={styles.cardContainer} ref={card => this.card = card}>
          ...
          <TouchableOpacity style={[styles.card_back,  styles.card]} onPress={() => Actions.biology10()}>
            {this.explanDisplay()}
          </TouchableOpacity>
        </CardFlip>
      </View>
...
export default connect(mapStateToProps, mapDispatchToProps)(Biology9);
